<template>
  <section>
    <NuxtLink :to="link"
      class="deck-card group absolute top-[6rem] left-1/2 -translate-x-1/2 w-[90vmin] sm:h-[55vmin] h-[80vmin] rounded-3xl overflow-hidden bg-green-900 shadow-xl transition-transform duration-300 hover:scale-[1.03]">
      <img :src="img" alt=""
        class="absolute inset-0 w-full h-full object-cover opacity-70 group-hover:opacity-50 transition duration-300" />
      <div
        class="relative z-10 p-6 h-full flex flex-col justify-end bg-gradient-to-t from-black/80 via-black/40 to-transparent text-white">
